...
For in-process cron jobs, scheduled workflows, and other trusted background work that has no JWT or human membership, pass the core FGA `systemActor` option on the specific agent, workflow, or tool invocation instead of adding a fake membership to the user. The request context must include an `organizationId`; Mastra denies system-actor FGA checks without tenant scope.
